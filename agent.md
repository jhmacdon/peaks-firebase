...
4. **Never add a legacy-datastore fallback after a migration.** If Cloud SQL is missing Firestore records or joins, repair the idempotent migration and the live writer, then verify counts and anti-joins. Missing rows must fail the audit; they must not trigger a Firestore read or a silent skip.
