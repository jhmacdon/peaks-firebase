 ...
 * (`cloud-sql/api/src/weather-refresh.ts`), run three times a day by the
 * Cloud Scheduler job `peaks-api-weather`. It covers a bounded set — a
 * destination with at least three sessions, or on a curated list — about
 * 776 destinations today, not the full catalog. Each entry is a Kelvin/mm/
 * m-s daily forecast from Open-Meteo, dated at local noon with an explicit
 * UTC offset.
 *
 ...
 * `buildDestinationWeatherStrip` in `../weather-forecast.ts` for the actual
 ...
 * empty read once per hour (the page's ISR window), which is negligible at
 * Firestore's per-read pricing and was accepted as-is rather than adding a
 * skip-list or a second cache layer for it. */
