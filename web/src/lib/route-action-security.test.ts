...
test("factory activation needs a live lease and the approved candidate binding", () => {
  assert.match(
    actions,
    /export async function analyzePendingRouteForFactory\([\s\S]*lease_token = \$3/
  );
  assert.match(
    actions,
    /export async function acceptRouteWithSegmentsForFactory\([\s\S]*acceptRouteWithSegmentsUnchecked\(id, activation\)/
  );
  assert.doesNotMatch(
    actions,
    /export async function (?:analyzePendingRoute|acceptRouteWithSegments)\([\s\S]{0,160}?factoryActivation/
  );
  assert.match(
    actions,
