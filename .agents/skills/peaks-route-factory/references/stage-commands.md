...
```bash
.agents/skills/peaks-route-factory/scripts/verify_standard_route.sh \
  --route-id <route-id> --destination-id <destination-id> \
  --trailhead-id <trailhead-id> \
  > /private/tmp/peaks-route-worker/<destination-id>-verify.json

npm --prefix cloud-sql/migrate run routes:jobs -- \
  transition --destination-id <destination-id> --lease-token <lease-token> \
  --to verified --route-id <route-id> \
  --result-file /private/tmp/peaks-route-worker/<destination-id>-verify.json \
  --apply
```
