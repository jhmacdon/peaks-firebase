...
-- A route-cover audit catches old gaps. These deferred checks also keep any
-- active Peaks route from being created or left without a fully credited
-- derived cover. Deferral lets an importer swap covers or demote a route in
-- one transaction. Fixed infrastructure cost: $0/month.
...
BEGIN
  -- Serialize cover edits through their shared route before checking the final
  -- state. A later concurrent commit then sees the earlier cover change.
