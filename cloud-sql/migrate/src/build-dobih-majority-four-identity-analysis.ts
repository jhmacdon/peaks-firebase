/** Builds a fail-closed identity analysis for the queued DoBIH majority-four rosters. */
...
const CATALOG_FILE = "dobih-majority-four-catalog-2026-08-31.csv";
const INPUT_SHA256 = {
  majorityCandidates:
  ...
  catalog:
    "897feb3c3d0bf132694cfcb7455bb43a6b7ad7049f3fbd486bfd16244bfbe8aa",
} as const;

const EXPECTED_CATALOG_ROWS = 2_524;
const EXPECTED_MEMBERSHIPS = 1_627;
const EXPECTED_IDENTITIES = 1_015;
const EXPECTED_PRIOR_IDENTITIES = 317;
const EXPECTED_PRIOR_EXPLICIT = 120;
const EXPECTED_NEW_IDENTITIES = 698;
...
const ACCESS_BLOCKS = [
  {
    ...
    name: "Pillar Rock",
    kind: "technical_climb_optional_member",
    policy:
      "Do not publish a hiking route. Add exact optional-member support or a reviewed " +
      "technical-climb model before this list can publish.",
    evidenceUrl: "https://ldwa.org.uk/hillwalkers/register2.php",
  },
  {
    sourceMemberId: "dobih:2711",
    name: "Mickle Fell",
    kind: "controlled_firing_range_access",
    policy:
      "Any later route must use approved access and carry the current range warning.",
    evidenceUrl: "https://www.gov.uk/guidance/north-england-public-access-to-military-areas",
  },
  {
  ...
  },
  {
    ...
    kind: "controlled_firing_range_access",
    policy:
      "Any later route must use guaranteed public access and carry the current range warning.",
    evidenceUrl:
...
export interface DobihMajorityFourIdentityAnalysis {
  schemaVersion: 1;
  generatedAt: string;
  reviewedAt: string;
...
export interface DobihMajorityFourIdentityAnalysisInputs {
...
function parseCatalog(bytes: Buffer): CatalogRow[] {
  ...
  const expectedHeaders = [
    "id", "name", "elevation_m", "lat", "lng", "osm_id", "external_ids",
  ...
  const rows = matrix.slice(1).map((values, index): CatalogRow => {
    ...
    const osmId = record.osm_id.trim() || null;
    if ((normalizedExternalIds.osm ?? null) !== osmId) {
    ...
    return {
      id,
      name,
      elevationM: parseFinite(record.elevation_m, `Catalog destination ${id} elevation`),
      lat,
      lng,
...
function sourceSelectionAllows(sourceKey: string, row: ParsedDobihRow): boolean {
  ...
  if (sourceKey === "dobih-birketts") return row.flags.B && row.country === "E";
  if (sourceKey === "dobih-synges") return row.flags.Sy && row.country === "E";
  ...
  for (const definition of DOBIH_MAJORITY_FOUR_KEEPER_LISTS) {
    for (const member of fixture.lists[definition.sourceKey].rows) {
      ...
      if (previous != null && canonicalJson(memberIdentity(previous)) !==
        ...
        throw new Error(`Majority-four source identity ${member.sourceMemberId} changed`);
      }
...
export function buildDobihMajorityFourIdentityAnalysis(
  inputs: DobihMajorityFourIdentityAnalysisInputs
): DobihMajorityFourIdentityAnalysis {
  assertSha256(inputs.majorityCandidateBytes, INPUT_SHA256.majorityCandidates,
    "Majority-four candidate fixture");
  ...
  validateKeeperFixture(majorityFixture, DOBIH_MAJORITY_FOUR_KEEPER_LISTS);
  for (const context of contexts) {
  ...
  if (sourceProvenance?.name !== DOBIH_SOURCE_NAME ||
    ...
    throw new Error("Majority-four DoBIH source provenance changed");
  }
  for (const context of contexts) {
  ...
  if (membershipCount !== EXPECTED_MEMBERSHIPS || members.size !== EXPECTED_IDENTITIES) {
    throw new Error(
      `Majority-four scope is ${membershipCount}/${members.size}; expected ` +
  ...
  for (const [sourceMemberId, member] of members) {
    ...
    if (raw == null || raw.country === "I" ||
      ...
      throw new Error(`Majority-four source ${sourceMemberId} changed from the pinned DoBIH CSV`);
    }
    for (const sourceKey of owners.get(sourceMemberId) ?? []) {
      if (!sourceSelectionAllows(sourceKey, raw)) {
        throw new Error(`Majority-four source ${sourceMemberId} fails ${sourceKey}'s country rule`);
      }
    }
  }
  ...
  if (reusedIds.length !== EXPECTED_PRIOR_IDENTITIES) {
    throw new Error(`Prior overlap has ${reusedIds.length} identities; expected 317`);
  }
  ...
  for (const sourceMemberId of reusedIds.sort((left, right) =>
    ...
    for (const owner of prior) {
      if (canonicalJson(memberIdentity(owner.member)) !== canonicalJson(memberIdentity(member))) {
        throw new Error(`Prior source ${sourceMemberId} changed before majority-four reuse`);
      }
    }
  ...
  if (newIds.length !== EXPECTED_NEW_IDENTITIES) {
    throw new Error(`New identity scope has ${newIds.length} rows; expected 698`);
  }
  ...
  for (const sourceMemberId of newIds) {
    ...
    const ownerCandidates = ownerKeys.map((sourceKey) => {
      const definition = DOBIH_MAJORITY_FOUR_KEEPER_LISTS.find((candidate) =>
        candidate.sourceKey === sourceKey
      );
      if (definition == null) throw new Error(`Missing majority-four definition ${sourceKey}`);
  ...
  if (sourceNeighborPairs.length !== 6) {
    throw new Error(
      `Majority-four source neighbor review has ${sourceNeighborPairs.length} pairs; expected 6`
    );
  }
  ...
  return {
    ...
    matchingRules: {
      ...
      duplicateGuard:
        "Every catalog summit and every other majority-four source identity within 150 m is " +
...
async function main(): Promise<void> {
  ...
  const analysis = buildDobihMajorityFourIdentityAnalysis({
    majorityCandidateBytes,
    baseCandidateBytes,
    baseResolutionBytes,
    openEightCandidateBytes,
