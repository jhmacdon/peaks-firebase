...
test("the queue publisher uses its bound lease for analysis and activation", () => {
  assert.match(
    publisher,
    /analyzePendingRouteForFactory\(\s*routeId,\s*factoryActivation\s*\)/
  );
  assert.match(
    publisher,
    /acceptRouteWithSegmentsForFactory\(\s*routeId,\s*factoryActivation\s*\)/
  );
  assert.doesNotMatch(
    publisher,
    /\banalyzePendingRoute\(\s*leaseToken|\bacceptRouteWithSegments\(\s*leaseToken/
  );
  assert.match(
    routeActions,
    /analyzePendingRouteForFactory[\s\S]*lease_token = \$3/
  );
