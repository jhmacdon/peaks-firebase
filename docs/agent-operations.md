# Peaks backend operations

Paths and commands are relative to the repository root. Read only the relevant sections.
...
## Build & Lint
For code or build-configuration changes, verify the affected project compiles and lints cleanly. Prose-only edits need diff and link checks:
...
After a push that triggers CI, inspect the run for that branch and commit. Fix failures caused by the change. If Actions cannot run because of billing or service failure, report that limit and the local checks; do not wait indefinitely or call the change deployed. For a deployment, verify the matching deploy run and live service before reporting success.
